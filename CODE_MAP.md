...
| Hạng mục | Chi tiết |
|---|---|
| Xử lý Backend | [`app/Http/Controllers/Api/V1/EmployeeController.php`](app/Http/Controllers/Api/V1/EmployeeController.php) (`index`/`show`/`store`/`update`/`destroy`, `show` dùng Route Model Binding), [`app/Services/EmployeeService.php`](app/Services/EmployeeService.php) (`generateCode()` tự sinh mã `NV001`..., chặn vòng lặp quản lý qua `wouldCreateCycle()` khi đổi `manager_id`), [`app/Repositories/EmployeeRepository.php`](app/Repositories/EmployeeRepository.php) (`wouldCreateCycle()`, `isSuperiorOf()` — cả 2 đều duyệt ngược chuỗi `manager`) |
...
| Validation | [`app/Http/Requests/Employee/StoreEmployeeRequest.php`](app/Http/Requests/Employee/StoreEmployeeRequest.php), [`app/Http/Requests/Employee/UpdateEmployeeRequest.php`](app/Http/Requests/Employee/UpdateEmployeeRequest.php) — **không** có rule cho `code` (tự sinh, xem Ghi chú). `avatar` **cố ý chưa validate** ở đây, để dành xử lý upload riêng ở Ngày 24. Rule `unique` (`company_email`, `cccd`, `personal_tax_code`, `user_id`) ở bản Update đều loại trừ chính bản ghi đang sửa qua `$this->route('employee')->id`, giống `UpdateDepartmentRequest` |
| Output / Phân quyền hiển thị | [`app/Http/Resources/EmployeeResource.php`](app/Http/Resources/EmployeeResource.php) — **ẩn field nhạy cảm** (`date_of_birth`, `gender`, `phone`, `personal_email`, `cccd`, `personal_tax_code`, `addresses`) khi người xem (`$request->user()->employee`) là cấp dưới (ở bất kỳ cấp nào) của nhân viên đang xem, dùng `EmployeeRepository::isSuperiorOf()`. Field `manager` lồng bên trong **phải bọc đệ quy qua `EmployeeResource`** (không trả Model trần) để logic ẩn field áp dụng cả cho quan hệ lồng — nếu quên, dữ liệu nhạy cảm của cấp trên sẽ lộ nguyên vẹn qua field `manager` dù xem trực tiếp thì bị ẩn đúng. `loadMissing(['department','position','manager'])` ở đầu `toArray()` vì `show()` (Route Model Binding) và `store()`/`update()` không tự eager-load như `index()` |
...
| Database & API | Endpoint `GET/POST /api/v1/employees`, `GET/PUT/DELETE /api/v1/employees/{employee}` — khai báo tại [`routes/api/v1/employees.php`](routes/api/v1/employees.php). Khác Department: **4 mã quyền riêng** cho từng hành động (`employee.view`, `employee.create`, `employee.update`, `employee.delete`), không gộp chung 1 mã `manage` |
| Ghi chú | Ngày 21 Model, Ngày 22 Validation, Ngày 23 CRUD + Resource — xong, đã test qua HTTP thật (không chỉ đọc code). `code` **tự sinh** giống `PB001` của Department. `personal_email` không bắt `unique` (không có ràng buộc DB, chỉ là liên hệ phụ). Quyết định đã chốt: **không** chặn xóa nhân viên còn cấp dưới (khác Department) — cấp dưới chỉ đơn giản không xem được `manager` nữa sau khi cấp trên bị xóa mềm, do `SoftDeletingScope` ẩn bản ghi đã xóa, không dựa vào `nullOnDelete()` của DB (không kích hoạt với xóa mềm) |
| Danh sách tệp cần sửa khi bảo trì | Toàn bộ các file trên; Ngày 24 sẽ xử lý upload `avatar` + PDF hợp đồng — nhớ thêm rule validate `avatar` trở lại vào 2 Request lúc đó; Ngày 25 tối ưu N+1/index — chú ý `isSuperiorOf()`/`EmployeeResource` lồng đệ quy có thể phát sinh nhiều query khi chuỗi quản lý sâu, đáng xem lại |
