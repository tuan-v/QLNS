...
| Hợp đồng | [`EmployeeContractController.php`](app/Http/Controllers/Api/V1/EmployeeContractController.php)/Service/Repository — chỉ Tạo+Xem+Tải (không Sửa/Xóa), IDOR check ở `download()` |
...
| Test | [`EmployeeTest.php`](tests/Feature/Employee/EmployeeTest.php), [`EmployeeContractTest.php`](tests/Feature/Employee/EmployeeContractTest.php), [`EmployeeDocumentTest.php`](tests/Feature/Employee/EmployeeDocumentTest.php), [`EmployeeTransferTest.php`](tests/Feature/Employee/EmployeeTransferTest.php) |
...
| Xử lý Backend | [`AttendanceController.php`](app/Http/Controllers/Api/V1/AttendanceController.php) (`checkIn`/`checkOut`/`today`/`mine`/`index`), [`AttendanceService.php`](app/Services/AttendanceService.php) (khớp điểm/ca, tính trễ-sớm-OT), [`AttendanceRepository.php`](app/Repositories/AttendanceRepository.php), [`AttendanceLogRepository.php`](app/Repositories/AttendanceLogRepository.php) |
...
| Validation | [`CheckInRequest.php`](app/Http/Requests/Attendance/CheckInRequest.php)/[`CheckOutRequest.php`](app/Http/Requests/Attendance/CheckOutRequest.php) |
| Database & API | `attendances` (unique `employee_id`+`attendance_date`+`work_shift_id` — 1 dòng/CA/ngày, cho phép nhiều ca cùng ngày), `attendance_logs`; `POST /check-in`, `/check-out`, `GET /today`, `/me`, `/` (HR) ([`routes/api/v1/attendances.php`](routes/api/v1/attendances.php)); quyền `attendance.check`/`view_own`/`view_all` |
| Test | [`AttendanceTest.php`](tests/Feature/Attendance/AttendanceTest.php) |
...
| Ghi chú | `matchLocation()` không khớp điểm nào (wifi/gps/qr) thì **vẫn cho chấm công**, chỉ đánh dấu `status=needs_review` (không chặn cứng). OT tính từ phần **SAU `end_time`**, không phải `actual - standard`. **Khung giờ check-in** (Ngày 44 — bug người dùng phát hiện): trước đó không so giờ hiện tại với giờ ca, cho phép chấm công 1 ca cách xa giờ thật (vd chấm Ca chiều lúc 8h sáng) — đã sửa bằng `assertWithinCheckInWindow()` (chỉ cho vào sớm tối đa 30 phút trước `start_time`, chặn hẳn sau `end_time`, dùng "Xin bổ sung chấm công" thay thế, mục 17). WiFi "mất điểm" khi test qua Docker Desktop là do NAT (source IP bị đổi thành gateway container), không phải bug — dev local cần đặt `allowed_ip_cidr` theo đúng subnet Docker, **đổi lại khi triển khai thật** |
| Danh sách tệp cần sửa khi bảo trì | Toàn bộ các file trên |
...
| Hạng mục | Chi tiết |
| --- | --- |
| Giao diện Frontend | [`CheckIn.vue`](resources/js/views/Attendance/CheckIn.vue) — "Xin điều chỉnh" (sửa bản ghi có sẵn), "Xin bổ sung chấm công" (tạo bản ghi cho ca+ngày chưa từng chấm công), "Xin miễn trừ đi muộn" (mục 24); [`AttendanceAdjustments.vue`](resources/js/views/Attendance/AttendanceAdjustments.vue) — HR duyệt, dùng `confirm.input` của `DataTable.vue` |
| Xử lý Backend | [`AttendanceAdjustmentController.php`](app/Http/Controllers/Api/V1/AttendanceAdjustmentController.php) (`store`/`mine`/`index`/`decide`), [`AttendanceAdjustmentService.php`](app/Services/AttendanceAdjustmentService.php) (`requestForEmployee()`/`decide()` rẽ nhánh theo `type`: `correction`/`supplement`/`excuse`), [`AttendanceAdjustmentRepository.php`](app/Repositories/AttendanceAdjustmentRepository.php) |
| Model | [`AttendanceAdjustment.php`](app/Models/AttendanceAdjustment.php) — `employee_id`/`work_shift_id`/`attendance_date` set cho cả 3 loại (đọc trực tiếp không cần qua `attendance`) |
...
| Test | [`AttendanceAdjustmentTest.php`](tests/Feature/Attendance/AttendanceAdjustmentTest.php) |
| Ghi chú | Duyệt `correction`/`supplement` thì áp giờ đề xuất vào `Attendance` qua `AttendanceService::applyAdjustment()` (tái dùng công thức trễ/sớm/OT), `status=completed` chỉ khi đủ cả giờ vào lẫn ra. Duyệt `excuse` **chỉ** set `late_excused=true`, không đổi giờ (mục 24). Từ chối ở loại nào cũng không đụng `Attendance` |
| Danh sách tệp cần sửa khi bảo trì | Toàn bộ các file trên |
...
| Ghi chú | Nhãn suy theo thứ tự ưu tiên `on_leave` > `late` > `insufficient` > `full`/`absent` — đơn nghỉ phép `approved` (không phải `hourly`) tự che ngày thành `on_leave`, loại khỏi cả "Vắng" lẫn thống kê giờ công |
| Danh sách tệp cần sửa khi bảo trì | Toàn bộ các file trên |
...
| Xử lý Backend | [`PayrollRepository.php`](app/Repositories/PayrollRepository.php), [`PayrollDetailRepository.php`](app/Repositories/PayrollDetailRepository.php), [`PayrollService.php`](app/Services/PayrollService.php) (`generateForPeriod()` tự tính lương cho toàn bộ nhân viên active+probation có hợp đồng active — kéo dữ liệu chấm công/nghỉ phép/hợp đồng thật; `close()`/`markAsPaid()` state machine processing→closed→paid), [`AttendanceService::sumOvertimeMinutesForEmployee()`](app/Services/AttendanceService.php) (hàm mới, không đổi API cũ), [`PersonalIncomeTaxCalculator.php`](app/Services/Payroll/PersonalIncomeTaxCalculator.php) (biểu thuế TNCN lũy tiến 7 bậc, tách riêng để unit-test độc lập) |
| Giao diện API | [`PayrollController.php`](app/Http/Controllers/Api/V1/PayrollController.php) (`index`/`show`/`generate`/`close`/`markAsPaid`), [`GeneratePayrollRequest.php`](app/Http/Requests/Payroll/GeneratePayrollRequest.php), [`PayrollResource.php`](app/Http/Resources/PayrollResource.php)/[`PayrollDetailResource.php`](app/Http/Resources/PayrollDetailResource.php), route [`routes/api/v1/payrolls.php`](routes/api/v1/payrolls.php) — quyền `payroll.view_all`/`payroll.manage` (HR+Admin) đã có sẵn từ Ngày 08, không cần thêm permission mới |
| Test | [`PersonalIncomeTaxCalculatorTest.php`](tests/Unit/Services/Payroll/PersonalIncomeTaxCalculatorTest.php) (6 test Unit thuần), [`PayrollTest.php`](tests/Feature/Payroll/PayrollTest.php) (11 test — gọi thẳng `PayrollService`, số liệu kịch bản chính đối chiếu đúng tay tính qua tinker), [`PayrollControllerTest.php`](tests/Feature/Payroll/PayrollControllerTest.php) (16 test — đi qua HTTP thật + phân quyền theo role) |
| Ghi chú — các giả định cần xác nhận lại trước khi dùng số liệu thật | (1) Mốc bậc thuế TNCN + giảm trừ bản thân 11tr + bảo hiểm 10.5% là số liệu **pháp luật**, có thể đã lỗi thời — chưa kiểm tra lại quy định hiện hành. (2) Chưa trừ giảm trừ người phụ thuộc (Employee/Contract chưa có cột lưu số người phụ thuộc). (3) `STANDARD_DAILY_HOURS=8` dùng chung cho mọi nhân viên, chưa lấy riêng theo `work_shifts.standard_work_minutes` từng ca. (4) OT tính đồng giá 150%, chưa phân biệt ngày thường/cuối tuần/lễ. (5) `total_payroll_amount` = tổng `net_salary`, không phải gross. (6) Bảng `holidays` đang rỗng, chưa có CRUD/seeder — `standard_work_days` hiện chỉ trừ T7/CN, chưa trừ được ngày lễ thật nào. (7) `generate()` cố tình KHÔNG trả kèm `details` trong JSON (payroll có thể nhiều trăm nhân viên) — khác `show()` có tải kèm; Frontend gọi xong `generate()` phải tự `GET /payrolls/{id}` nếu cần xem chi tiết ngay |
| Bug đã sửa | `PayrollController::generate()` ban đầu dùng `response()->json(new PayrollResource($payroll), 201)` — cách này KHÔNG đi qua cơ chế bọc `"data"` tự động của `JsonResource`, khiến response thiếu key `data` so với 3 hàm còn lại trong cùng Controller. Phát hiện qua `PayrollControllerTest.php` (không phải đọc code đoán ra), sửa thành `(new PayrollResource($payroll))->response()->setStatusCode(201)` |
| Chưa làm | Frontend `PayrollList.vue`/`PayrollDetail.vue` (Ngày 49-50), quản lý `salary_components`/`holidays` qua CRUD |
| Danh sách tệp cần sửa khi bảo trì | Toàn bộ các file trên |
