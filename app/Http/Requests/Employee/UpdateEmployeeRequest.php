...
class UpdateEmployeeRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'personal_email' => ['required', 'email', 'unique:employees,personal_email,' . $this->route('employee')->id],
            'phone' => ['required', 'string', 'max:10', 'unique:employees,phone,' . $this->route('employee')->id],
            'date_of_birth' => ['required', 'date', 'before:today'],
            'gender' => ['required', 'in:male,female,other'],
            'cccd' => ['required', 'digits:12', 'unique:employees,cccd,' . $this->route('employee')->id],
            'personal_tax_code' => ['required', 'string', 'max:20', 'unique:employees,personal_tax_code,' . $this->route('employee')->id],
